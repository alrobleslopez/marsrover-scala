...
object MoveForward extends Command {
  override def execute(position: Position): Position = {
    ...
    position.direction match {
      case Direction.NORTH => Position(position.x, position.y + 1, Direction.NORTH)
      case Direction.SOUTH => Position(position.x, position.y - 1, Direction.SOUTH)
      case Direction.EAST => Position(position.x + 1, position.y, Direction.EAST)
      case Direction.WEST => Position(position.x - 1, position.y, Direction.WEST)
    }
  }
}
